...
Mathematically, this can be done using the <%= link 'reference/math/dot-product', 'dot product' %>: given the ray origin stem:[E], the hit position stem:[P]
...
``group_id`` represents which group the intersected primitive is part of. This is important for <</extensions/renderers/cartoon/explanations#,edge detection>>.

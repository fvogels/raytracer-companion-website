...
As <</reference/design/primitives/basics/explanations#,explained here>>, transformations are actually faked.
...
When we create a matrix for a translation by the vector stem:[\vec v(x,y,z)], we get
...
What is the inverse operation of moving everything a distance stem:[(x, y, z)]? Simple:
it's moving it back a distance stem:[(-x, -y, -z)]. Finding the matrix for this transformation is very easy:
...
To make this trick possible, we need to build this inverse matrix stem:[T^{-1}] immediately when building the regular stem:[T] matrix.
...
For this reason, we introduced the class ``Transformation``. This class pairs up both the
matrix stem:[T] and its inverse stem:[T^{-1}]. Whenever we need a translation transformation, we set up both matrices and
group them in a ``Transformer`` object.

== Transformer Primitive

Lastly, we need to define a scaling ``Primitive``.
The ``Transformer`` class has already been written for you and takes care of most of the complexity.
It takes a ``Transformation`` object and uses it to transform and "untransform" rays and hit locations.

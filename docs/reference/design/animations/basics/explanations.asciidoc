...
* ``Animation&lt;T&gt;`` models an animation of a ``T``. For example ``Animation&lt;double&gt;``
  represents a ``double`` animation, but you could also animate ``Point3D``s, ``Color``s, etc.
...
you have no other choice than to rely on a regular factory function: ``TimeStamp::from_seconds(x)``.
====
...
* ``TimeStamp::zero()`` returns the zero time stem:[T_0].

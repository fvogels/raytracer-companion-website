% magick -density 2400 $input -resize 640x $output
...
\begin{tikzpicture}
  ...
  \draw[fill=white] (light) circle [radius=0.05cm] node [white,above] {$L$};
  \draw[white] (light) -- (hit);

  \foreach \x in {10,30,...,170} {
    \draw[red,-latex] (hit) -- ++(\x:0.5);
  }

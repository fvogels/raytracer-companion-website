...
* The scripting language itself does not support templates, so we will need to expose `ease_animation` repeatedly, once for each animatable type `T`.
  By default, your ray tracer can only animate `double`s, but you can also add support for <</extensions/animations/point-animation/explanations#,point animations>> and <</extensions/animations/angle-animation/explanations#,angle animations>>.
  For each type `T` you defined animations for, use the `BIND_AS` macro to expose `ease_animation<T>` as `ease` (e.g. `BIND_AS(ease_animation<double>, ease)`).

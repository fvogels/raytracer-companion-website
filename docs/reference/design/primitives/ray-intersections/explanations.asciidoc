...
it returns a list of _all_ intersections. However, in practice,
...
The <</reference/math/misc/ray/explanations#,``t``-value>> indicates where on the ray the intersection occurs.
...
Mathematically, this can be done using the <</reference/math/linear-algebra/dot-product/explanations#,dot product>>: given the ray origin stem:[E], the hit position stem:[P]

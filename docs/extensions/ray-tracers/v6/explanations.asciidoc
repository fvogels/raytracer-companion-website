...
[TASK]
====
Implement ``RayTracerV6``.

* ``RayTracerV5`` should have two ``trace`` methods: one with and one without ``weight`` parameter.
  For ``RayTracerV6``, you'll need to override the one with the ``weight`` parameter.
* ``RayTracerV5::trace`` calls ``compute_ambient``, ``process_lights`` and

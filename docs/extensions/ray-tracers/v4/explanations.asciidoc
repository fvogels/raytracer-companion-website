...
The easiest way to check if a hit occurs between stem:[L] and stem:[P] is to rely on the stem:[t]-value of the hit.
...
* If there's a hit where stem:[0 \leq t < 1], return black.
* Call ``RayTracerV3::process_light_ray`` and return its result.
====
...
causing some "noise" to appear on your renderings. To remedy this, you will have to slightly change the stem:[0 \leq t < 1] to account
for the approximation errors.
====

[TASK]
====
Integrate ``RayTracerV4`` with the rest of the ray tracer.

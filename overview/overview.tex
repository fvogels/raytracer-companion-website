...
\begin{tikzpicture}[extension/.style={draw,minimum width=5cm,minimum height=1cm,font=\scshape,rounded corners=4pt,drop shadow},
  ...
  \begin{pgfonlayer}{main}
    ...
    \node[extension,anchor=north,easy] (modifying basic sample) at ($ (start.south) + (-13,-1) $) {\html{extensions/modifying-basic-sample}{Basic Sample}};
    \draw[dependency] (modifying basic sample) |- (start);

    \node[extension,anchor=north,easy] (scripting basics) at ($ (start.south) + (-7,-1) $) {\html{extensions/scripting/basics}{Basic Scripting}};
    \draw[dependency] (scripting basics) |- (start);

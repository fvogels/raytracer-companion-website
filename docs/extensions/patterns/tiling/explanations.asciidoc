...
[TASK]
====
Use `tiling_y` to create a pattern `lines(thickness, spacing)` for horizontal lines together with `xsplit` or `ysplit` from the <</extensions/patterns/lambda/explanations#,lambda pattern extension>>.

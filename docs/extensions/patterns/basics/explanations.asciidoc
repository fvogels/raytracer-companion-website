...
[TASK]
====
Take a good look at the types ``Primitive`` and ``PrimitiveImplementation``, as you will want to use it as a guide to implement patterns.
...
Place the code in files named `patterns/constant-pattern.h`
...
For example, `constant2d(true)` should return `true` for all stem:[(x, y)].
====

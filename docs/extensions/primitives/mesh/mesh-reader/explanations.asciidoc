...
[IMPORTANT]
=====
Make sure to check that the file opened successfully.
If the file failed to open, you will still be able to read data from it, but this data will be garbage.
=====
====
...
[TASK]
====
Show that if given a wrong path, this error is caught and a message `Failed to open file at <path>` is shown in the logs.
You can use the `CHECK` macro for this.
====

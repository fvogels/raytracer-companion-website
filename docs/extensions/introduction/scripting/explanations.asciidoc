...
Here, ``Animations.animate(0, 5, seconds(10))`` means "we wish to have an animated `double` `x` that goes from `0` to `5` in 10 seconds".
...
There is no evaluation for this extension, meaning there is no need to save anything.
No questions will be asked about this step during the defense.

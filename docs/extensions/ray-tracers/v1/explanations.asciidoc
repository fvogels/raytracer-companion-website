...
Let's first take a look at its current implementation (i.e., the one you copied from `RayTracerV0::trace`).
...
There's also the `RGB` function that allows you to specify colors using their R, G, B components as floating point values between 0 and 1, e.g., `RGB(0.1, 0.2, 0.3)`.
=====
====

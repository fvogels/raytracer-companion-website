...
Add extra parameters for both to the constructor.
====
...
[TASK]
====
Implement ``RayTracerV3::process_light_ray`` using the pseudocode below as guide.
...
[TASK]
====
Add the necessary code in `scripting/materials-module.cpp` so as to be able to use specular lighting in scripts.
====

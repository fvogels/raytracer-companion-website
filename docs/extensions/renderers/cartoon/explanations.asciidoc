...
The only thing left to do is make the change to `CartoonRenderer` that achieves the effect shown above.
Examine ``CartoonRenderer``'s current source code and make play around with it to check whether your understanding is correct.
...
[TIP]
=====
You only need an extra method call somewhere.
The math part has already been implemented for you somewhere.
=====
====

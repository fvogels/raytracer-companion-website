...
* The scripting language itself does not support templates, so we will need to expose `ease_animation` repeatedly, once for each animatable type `T`.
  By default, your ray tracer can only animate `double`s, but you can also add support for <</extensions/animations/point-animation#,point animations>> and <</extensions/animations/angle-animation#,angle animations>>.

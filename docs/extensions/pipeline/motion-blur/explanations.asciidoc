...
Another trick that you can apply is to give subframes different weights.
E.g., you may want to count the last subframe twice, so that it stands out more.
...
=====
Say we have subframes 1, 2, 3, ..., 150 and we use `motion_blur(5, 2)`.
This causes the frames to be built as follows:
